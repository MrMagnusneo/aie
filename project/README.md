...
- `src/static/style.css` - современный адаптивный дизайн Web GUI (темная тема, glassmorphism).
...
Интерактивный Web GUI предоставляет следующие возможности:
- **Интерактивная доска:** Вы можете выбирать свои фигуры кликом и делать ходы, нажимая на подсвеченные легальные поля (включая автоматическое превращение пешек).
- **Визуализация вероятностей ходов:** Легальные ходы подсвечиваются на доске цветовым градиентом в соответствии с оценкой ИИ. Лучший ход подсвечивается фиолетовым цветом.
- **Таблица ранжирования:** Справа отображается полный список легальных ходов, отсортированных по вероятности модели.
- **Режим игры против AI:** Возможность играть за белых, пока ИИ автоматически отвечает за черных.
- **Режим автоигры (Self-Play):** Наблюдение за игрой ИИ против самого себя с возможностью регулировать скорость ходов.
- **История и откат:** Вы можете отменять сделанные ходы кнопкой «Назад» и загружать любые позиции по FEN.
- **Метрики:** Прямо в интерфейсе отображаются метаданные модели и ее точность.
